...
class BugAnalyzer:
    def __init__(self, monitor, reference_set):
        ...
        self.total_output_set = self.total_output_set()

    def classify_inconsistent_cert(self):
    ...
    def __filter_off_limit_cert(self):
        incomplete_output_set = self.incomplete_output_set()
        inconsistent_cert_set = self.inconsistent_cert_set()
        ...
        tool.write_(tool.inconsistent_cert_with_feature_file(self.monitor, self.feature.output_limit.__name__),
                    cert_with_feature_set)

        cert_with_feature_set = {}
        for fingerprint in list(inconsistent_cert_set.keys()):
            cert = inconsistent_cert_set[fingerprint]
            if self.feature.service_delay(cert):
                inconsistent_cert_set.pop(fingerprint)
                cert_with_feature_set.update({fingerprint: cert})
        tool.write_(tool.inconsistent_cert_with_feature_file(self.monitor, self.feature.service_delay.__name__),
                    cert_with_feature_set)
        tool.write_(tool.inconsistent_cert_with_feature_file(self.monitor, "remainder"), inconsistent_cert_set)

    def __filter_by_incomplete_output_feature(self):
        incomplete_output_set = self.incomplete_output_set()
        inconsistent_cert_set = self.remainder_set()
        for feature_discriminant in self.feature.output_feature_list:
            ...
            for fingerprint in self.total_output_set:
                output = self.total_output_set[fingerprint]
                if feature_discriminant(output):
            ...
            if self.__discriminant_a(incomplete_output_with_feature_num, output_with_feature_num):
                ...
                for fingerprint in list(inconsistent_cert_set.keys()):
                    ...
                    if domain in bad_domain:
                        inconsistent_cert_set.pop(fingerprint)
                        cert_with_feature_set.update({fingerprint: cert})
                tool.write_(tool.inconsistent_cert_with_feature_file(self.monitor, feature_discriminant.__name__),
                            cert_with_feature_set)
        tool.write_(tool.inconsistent_cert_with_feature_file(self.monitor, "remainder"), inconsistent_cert_set)
    ...
    def inconsistent_cert_set(self):
        inconsistent_cert_set = {}
        folder = tool.folder(self.monitor, tool.INCONSISTENT_CERT_FOLDER)
    ...
    def incomplete_output_set(self):
        return tool.read_(tool.incomplete_output_set_file(self.monitor))

    def total_output_set(self):
        return tool.read_(tool.total_output_set_file(self.monitor))

    def remainder_set(self):
        return tool.read_(tool.inconsistent_cert_with_feature_file(self.monitor, "remainder"))
